...
import { frequencyJourney } from "@/lib/content/frequency";

/**
 * Erste Resonanzreise: drei Fragen als Einladung zur Kontemplation.
 *
 * Die Karten wirken bewusst hochwertig und interaktiv (Hover-Glow), tragen
 * aber in dieser Phase noch keine Funktion – daher reine Darstellungsebene
 * ohne interaktive Rolle, um keine Aktion zu versprechen.
 */
export function FrequencyJourney() {
  return (
    <Section aria-labelledby="frequency-journey-title">
      <SectionHeading
        eyebrow={frequencyJourney.eyebrow}
        title={frequencyJourney.title}
        id="frequency-journey-title"
      >
        <p className="font-serif text-xl italic text-violet-soft sm:text-2xl">
          {frequencyJourney.text}
        </p>
      </SectionHeading>

      <ul className="mt-16 grid gap-6 md:grid-cols-3">
        {frequencyJourney.questions.map((question) => (
          <li key={question.id}>
