...
import { consciousnessJourney } from "@/lib/content/consciousness";

/**
 * Geführte Reise: drei Fragen als Einladung zur Kontemplation.
 *
 * Die Karten wirken bewusst anklickbar (Hover-Glow), tragen aber in dieser
 * Phase noch keine Funktion – daher reine Darstellungsebene ohne
 * interaktive Rolle, um keine Aktion zu versprechen.
 */
export function ConsciousnessJourney() {
  return (
    <Section aria-labelledby="consciousness-journey-title">
      <SectionHeading
        eyebrow={consciousnessJourney.eyebrow}
        title={consciousnessJourney.title}
        id="consciousness-journey-title"
      >
        <p className="font-serif text-xl italic text-violet-soft sm:text-2xl">
          {consciousnessJourney.text}
        </p>
      </SectionHeading>

      <ul className="mt-16 grid gap-6 md:grid-cols-3">
        {consciousnessJourney.questions.map((question) => (
          <li key={question.id}>
